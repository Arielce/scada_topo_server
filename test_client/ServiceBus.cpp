...
int ServiceBus::RequestSync(unsigned char srvType, int msgType, const char* msgBuf, int msgLen)
{
	if (msgLen < 0)
	{
		return -1;
	...
	int sendMsgLen = sizeof(unsigned char) * 2 + sizeof(int) + msgLen;
	char* sendMsgBuf = new char[sendMsgLen];

	int offset = 0;
	memcpy(sendMsgBuf, (char*) &srvType, sizeof(unsigned char));
	offset += sizeof(unsigned char);
	memcpy(sendMsgBuf + offset, (char*) &g_endian, sizeof(unsigned char));
	offset += sizeof(unsigned char);
	memcpy(sendMsgBuf + offset, (char*) &msgType, sizeof(int));
	offset += sizeof(int);
