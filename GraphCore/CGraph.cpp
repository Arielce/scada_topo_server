...
namespace SCADA_ALG
{
CPropertyContainer::CPropertyContainer() : m_graph(NULL)
{
}
...
//CVertex
CVertex::CVertex() : m_vetexId(-1), m_iDegree(0)
{
}
CVertex::CVertex(const SCA_ND_ID id) : m_vetexId(id), m_iDegree(0)
{
}
CVertex::~CVertex()
{
...
//CEdge
CEdge::CEdge(const SCA_ID id) : m_sVertex(NULL), m_eVertex(NULL), m_bEdgeOpen(false), m_edgeId(id), m_bEdgeBorder(false)
{
}
CEdge::CEdge(CVertex *const s_vertex, CVertex *const e_vertex, const SCA_ID id) : m_sVertex(s_vertex), m_eVertex(e_vertex), m_bEdgeOpen(false), m_edgeId(id), m_bEdgeBorder(false)
{
}
CEdge::~CEdge()
{
...
CGraph::CGraph(const std::string &name) : _name(name), _island_base_no(1000000)
{
...
//sub graph
CSubGraph::CSubGraph(const std::string &name) : CGraph(name)
{
}
CSubGraph::~CSubGraph()
{
